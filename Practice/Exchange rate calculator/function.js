async function currency() {
  try {
    const response = await fetch(
      "http://api.exchangeratesapi.io/v1/latest?access_key=8dc037f1a62d2a67444273ae72c7f0c7"
    );
    ...
    function findanswer() {
      if (value1.value != "") {
        let unit =
          allvalues[array.indexOf(input2.value)] /
          allvalues[array.indexOf(input1.value)];
        document.getElementById(
          "answer"
        ).innerText = `1 ${input1.value} = ${unit} ${input2.value}`;
    ...
    document.getElementById("answer").innerText =
      "Sorry the API could not be loaded please check your internet connection";
  }
}
currency();
